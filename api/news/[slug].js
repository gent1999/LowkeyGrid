export default async function handler(req, res) {
  ...
  // Extract numeric ID from slug (e.g., "170-drake-new-album" -> "170")
  ...
  try {
    ...
    const response = await fetch(`${apiUrl}/api/lowkeygrid/articles/${articleId}`);
...
<body>
  ...
  <p><a href="/">Return to 2koveralls Home</a></p>
    ...
    const url = `https://www.2koveralls.com/news/${slug}`;
...
<head>
  ...
  ${article.tags && article.tags.length > 0 ? `<meta name="keywords" content="${article.tags.join(', ')}, hip hop, rap, music news">` : ''}
...
<body>
  ...
  <p><a href="/news/${slug}">Read full article</a></p>
</body>
</html>`;
    ...
    console.error('Error fetching news article for crawler:', error);
    res.status(500).send(`
<!DOCTYPE html>
<html lang="en">
<head>
